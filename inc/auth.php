 ...
 */
define( 'WROMOX_PT_3D', plugin_dir_path( __DIR__ ) );
...
class WROMOX_Auth {
	...
	public function __construct() {
		if ( function_exists( 'wromox_http' ) && ( isset( $_SERVER['HTTP_HOST'] ) && isset( $_SERVER['REQUEST_URI'] ) ) ) {
			$this->current_url = wromox_http() . '://' . "{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}";
		}
	...
	public function auth() {
		...
		$security_salt = apply_filters( 'wromox_opauth_salt', '2NlBUibcszrVtNmDnxqDbwCOpLWq91eatIz6O1O' );
		$callback_param = 'int_callback';
		$strategy       = array();
		switch ( $network ) {
			case 'google':
				$app_id     = wromox_get_opt( 'goo_app_id' );
				$app_secret = wromox_get_opt( 'goo_app_secret' );
			...
			case 'vkontakte':
				$app_id     = wromox_get_opt( 'vk_app_id' );
				$app_secret = wromox_get_opt( 'vk_app_secret' );
			...
			default:
				$app_id     = wromox_get_opt( 'fb_app_id' );
				$app_secret = wromox_get_opt( 'fb_app_secret' );
		...
		$config = array(
			...
			'strategy_dir'       => WROMOX_PT_3D . '/vendor/opauth/',
			'Strategy'           => $strategy,
		);
		...
		switch ( $opauth['auth']['provider'] ) {
			case 'Facebook':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with Facebook. Please, try again later.', 'wromox' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'Facebook doesn\'t provide your email. Try to register manually.', 'wromox' ), 'error' );
					return;
				}
			...
			case 'Google':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with Google. Please, try again later.', 'wromox' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'Google doesn\'t provide your email. Try to register manually.', 'wromox' ), 'error' );
					return;
				}
			...
			case 'VKontakte':
				if ( empty( $opauth['auth']['info'] ) ) {
					wc_add_notice( __( 'Can\'t login with VKontakte. Please, try again later.', 'wromox' ), 'error' );
					return;
				}
				...
				if ( empty( $email ) ) {
					wc_add_notice( __( 'VK doesn\'t provide your email. Try to register manually.', 'wromox' ), 'error' );
					return;
				}
	...
	public function register_or_login( $email ) {
		...
		wc_add_notice( sprintf( __( 'You are now logged in as <strong>%s</strong>', 'wromox' ), $user->display_name ) );
